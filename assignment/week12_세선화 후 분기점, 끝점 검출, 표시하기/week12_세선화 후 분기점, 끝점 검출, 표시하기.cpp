...
void FeatureExtractThinImage(BYTE * Image, BYTE * Output, int W, int H)
{
	for (int i = 1; i < H - 1; i++) { // Y좌표
		for (int j = 1; j < W - 1; j++) { // X좌표
			Output[i*W + j] = Image[i*W + j]; //박스가 사라져서 여기서복사함
		}
	}
	...
	for (int i = 1; i < H-1; i++) { // Y좌표
		for (int j = 1; j < W - 1; j++) { // X좌표
			...
			else if (count >= 3)//분기점 표시, 색깔만 바꿨음
			{
				Output[i*W + j] = 200;
			}
		}
...
//==================================================================================
	...
	*/
	//--------------------------------------------------
	//--------------------------------------------------
	//12주차 과제 세선화 후 분기점 및 끝점 검출 및 표시하기
	Dilation(Image, Output, W, H);
	Dilation(Output, Image, W, H);
	Dilation(Image, Output, W, H);
	Erosion(Output, Image, W, H);
	...
	Erosion(Output, Image, W, H); //팽창->침식: closing, 침식->팽창: opening
	InverseImage(Image, Image, W, H);
	zhangSuen(Image, Image, H, W);//thining 지문에서 분기점 끝점 구할때 씀 분기점: 나눠지는곳(H의 가운데 두점 등)
	FeatureExtractThinImage(Image, Output, W, H);

	//--------------------------------------------------
	//==================================================================================

	SaveBMPFile(hf, hInfo, hRGB, Output, hInfo.biWidth, hInfo.biHeight, "FeatureExtractThinImage.bmp");
	free(Image);
	free(Output);
	free(Temp);
	return 0;
